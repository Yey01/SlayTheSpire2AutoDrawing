...
from app_config.runtime_config import RuntimeConfigMixin
from models.lineart_pipeline import LineartPipelineMixin
from models.deepsketch_vectorizer import DeepSketchVectorizerMixin
from models.path_pipeline import PathPipelineMixin
from models.precision_probe import probe as probe_precision


class AutoSketchApp(RuntimeConfigMixin, LineartPipelineMixin, DeepSketchVectorizerMixin, PathPipelineMixin):
    def __init__(self, root):
        ...
        self.color_to_sketch_var = tk.BooleanVar(value=False)
        self.onnx_lineart_session = None
        self.onnx_lineart_model_path = self.resource_path(os.path.join("models", "line_drawing", "line-drawings.onnx"))
        self.onnx_lineart_max_side = 900
        self.onnx_lineart_enhance = False
        self.onnx_merge_enabled = True
        self.onnx_merge_gap = 5
        self.onnx_merge_angle = 32.0
        self.onnx_merge_rounds = 2
        self.onnx_merge_max_candidates = 8
        self.onnx_merge_skip_density = True
        self.onnx_clean_mode = "balanced"
        self.debug_onnx_merge_stats = None
        self.onnx_cover_enabled = True
        self.onnx_cover_mode = "off"
        self.onnx_cover_enable_strategies = {"pct35", "pct40", "otsu"}
        self.onnx_cover_dark_ratio_threshold = 0.26
        self.onnx_cover_min_raw_confidence = 0.30
        self.onnx_cover_width_threshold = 1.6
        self.onnx_cover_spacing = 3
        self.onnx_cover_min_area = 18
        self.onnx_cover_max_paths = 180
        self.onnx_cover_max_paths_per_region = 8
        self.onnx_cover_angle = 45
        self.onnx_cover_keep_skeleton_center = True
        self.onnx_cover_min_region_score = 12.0
        self.onnx_cover_direction_mode = "pca"
        self.onnx_cover_min_segment_length = 4
        self.debug_onnx_clean_strategy = None
        self.debug_onnx_cover_stats = None
        self.debug_onnx_cover_decision = None
        self.color_path_backend = "onnx_skeleton"
        self.deepsketch_enabled = False
        self.deepsketch_model_size = "full"
        self.deepsketch_device = "auto"
        self.deepsketch_repo_path = self.resource_path(os.path.join("models", "deepsketch"))
        self.deepsketch_model_dir = os.path.join(self.deepsketch_repo_path, "model")
        self.deepsketch_max_side = 512
        self.deepsketch_auto_reduce_on_oom = True
        self.deepsketch_oom_retry_sides = [448]
        self.deepsketch_sample_step = 1.5
        self.deepsketch_use_bezier = True
        self.deepsketch_target_points = 12000
        self.deepsketch_max_points = 18000
        self.deepsketch_min_path_length = 3.0
        self.deepsketch_min_point_spacing = 1.0
        self.deepsketch_simplify_epsilon = 0.8
        self.deepsketch_max_paths = 1800
        self.deepsketch_cache_enabled = True
        self.deepsketch_save_ordered_paths_json = True
        self.deepsketch_quality_draw_step = 1
        self.deepsketch_precision_var = tk.StringVar(value="medium")
        self.deepsketch_precision = "medium"
        self.debug_deepsketch_stats = None
        self._precision_plan = None
        self._deepsketch_progress = 0
        self._progress_poll_id = None
        self.image_size = (0, 0)
        self.config_file = "config.txt"
        self.runtime_config_file = self.resource_path("runtime_config.json")
        self.hotkey_handles = {}
        ...
        self._probe_precision()

        self.apply_hotkeys()
        self.root.bind_all("<KeyPress>", self.on_capture_keypress, add="+")
    ...
    def setup_ui(self):
        ...
        ttk.Button(btn_frame, text="📁 选择并上传图片", command=self.load_image).pack(side="left", padx=(0, 5))
        ttk.Button(btn_frame, text="▶ 开始推理", command=self.start_inference).pack(side="left")
        ...
        ttk.Checkbutton(
            img_frame, text="彩图转线稿(ONNX)",
            variable=self.color_to_sketch_var,
            command=self.mark_inference_dirty
        ).pack(pady=(2, 0))

        mode_frame = ttk.Frame(img_frame)
        mode_frame.pack(fill="x", padx=10, pady=(2, 0))
        ttk.Label(mode_frame, text="彩图路径模式:", font=("Microsoft YaHei", 8)).pack(side="left")
        self.color_path_mode_var = tk.StringVar(value="quick")
        ttk.Radiobutton(
            mode_frame, text="快速模式", variable=self.color_path_mode_var,
            value="quick", command=self._on_color_path_mode_change
        ).pack(side="left", padx=(4, 2))
        ttk.Radiobutton(
            mode_frame, text="高质量模式", variable=self.color_path_mode_var,
            value="quality", command=self._on_color_path_mode_change
        ).pack(side="left", padx=2)
        self.color_path_mode_label = ttk.Label(mode_frame, text="", foreground="gray", font=("Microsoft YaHei", 7))
        self.color_path_mode_label.pack(side="left", padx=4)

        precision_frame = ttk.Frame(img_frame)
        precision_frame.pack(fill="x", padx=10, pady=(4, 0))
        ttk.Label(precision_frame, text="高质量模式:", font=("Microsoft YaHei", 8)).pack(side="left")
        self.precision_status_label = ttk.Label(precision_frame, text="等待检测...", foreground="gray",
                                                 font=("Microsoft YaHei", 8))
        self.precision_status_label.pack(side="left", padx=(4, 2))
        ttk.Button(precision_frame, text="重新检测", command=self._on_reprobe_precision).pack(side="left", padx=2)
        self.precision_detail_label = ttk.Label(img_frame, text="", foreground="gray",
                                                font=("Microsoft YaHei", 7))
        self.precision_detail_label.pack(pady=(2, 0))

        detail_frame = ttk.Frame(img_frame)
        detail_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(detail_frame, text="墨迹阈值:").grid(row=0, column=0, sticky="w", pady=2)
        self.threshold_var = tk.IntVar(value=100)
        thresh_spin = ttk.Spinbox(detail_frame, from_=10, to=200, textvariable=self.threshold_var, width=5, command=self.mark_inference_dirty)
        thresh_spin.grid(row=0, column=2, padx=(0, 5))
        thresh_spin.bind('<Return>', self.mark_inference_dirty)
        ...
        thresh_scale.bind("<ButtonRelease-1>", self.mark_inference_dirty)

        ttk.Label(detail_frame, text="过滤短线(防噪点):").grid(row=1, column=0, sticky="w", pady=2)
        self.min_len_var = tk.IntVar(value=10)
        minlen_spin = ttk.Spinbox(detail_frame, from_=0, to=100, textvariable=self.min_len_var, width=5, command=self.mark_inference_dirty)
        minlen_spin.grid(row=1, column=2, padx=(0, 5))
        minlen_spin.bind('<Return>', self.mark_inference_dirty)
        ...
        minlen_scale.bind("<ButtonRelease-1>", self.mark_inference_dirty)
        ...
        self.deepsketch_progress_var = tk.IntVar(value=0)
        self.deepsketch_progress_bar = ttk.Progressbar(
            self.content_frame, variable=self.deepsketch_progress_var,
    ...
    def _handle_tk_hotkey(self, event):
        hotkey = self._event_to_hotkey(event)
        if not hotkey:
            return None

        hotkey = hotkey.lower()
        if hotkey == self._hk("start", "F9").lower():
            self.on_hotkey_start()
            return "break"
        if hotkey == self._hk("pause", "F8").lower():
            self.on_hotkey_pause()
            return "break"
        if hotkey == self._hk("stop", "F10").lower():
            self.on_hotkey_stop()
            return "break"
        return None
    ...
    def on_capture_keypress(self, event):
        if self.hotkey_capture_target is None:
            return self._handle_tk_hotkey(event)
    ...
    def _set_hotkeys(self, start, pause, stop):
        self._clear_hotkeys()
        try:
            h_start = keyboard.add_hotkey(start, lambda: self.root.after(0, self.on_hotkey_start))
            h_pause = keyboard.add_hotkey(pause, lambda: self.root.after(0, self.on_hotkey_pause))
            h_stop = keyboard.add_hotkey(stop, lambda: self.root.after(0, self.on_hotkey_stop))
...
mist_scroll = {self.mist_scroll_var.get()}"""
    ...
    def _on_color_path_mode_change(self):
        mode = self.color_path_mode_var.get()
        if mode == "quality":
            self.color_path_backend = "deepsketch_full"
            self.deepsketch_enabled = True
            self.color_path_mode_label.config(text="(DeepSketch full, 解析较慢但质量高)", foreground="darkorange")
            self._probe_precision()
        else:
            self.color_path_backend = "onnx_skeleton"
            self.deepsketch_enabled = False
            self.color_path_mode_label.config(text="", foreground="gray")
        self.mark_inference_dirty()

    def _probe_precision(self):
        image_hw = None
        if self.image_path and self.image_size and self.image_size[0] > 0:
            image_hw = (self.image_size[1], self.image_size[0])
        cpu_budget = getattr(self, "deepsketch_cpu_max_time_seconds", 120.0)
        try:
            plan = probe_precision(image_hw=image_hw, cpu_budget_s=cpu_budget)
        except Exception:
            plan = None
        self._precision_plan = plan
        self._update_precision_ui()
        if plan is not None:
            self.apply_deepsketch_plan(plan)

    def apply_deepsketch_plan(self, plan):
        self.deepsketch_device = plan.primary_device
        self.deepsketch_max_side = plan.primary_max_side
        self.deepsketch_oom_retry_sides = plan.gpu_fallback_sides
        self.deepsketch_auto_reduce_on_oom = True

    def apply_deepsketch_precision(self):
        if self._precision_plan is not None:
            self.apply_deepsketch_plan(self._precision_plan)

    def _update_precision_ui(self):
        plan = self._precision_plan
        if plan is None:
            self.precision_status_label.config(text="检测失败", foreground="red")
            self.precision_detail_label.config(text="")
            return
        if plan.is_gpu:
            if plan.uses_shared_memory:
                self.precision_status_label.config(
                    text=f"{plan.gpu_name or 'GPU'} {plan.primary_max_side}px (含共享内存)",
                    foreground="darkorange"
                )
                detail = (
                    f"专用仅 {plan.dedicated_free_gib:.1f}G/{plan.dedicated_total_gib:.1f}G 空闲, "
                    f"共享 {plan.shared_available_gib:.1f}G 可用 | "
                    f"降级: {plan.gpu_fallback_sides or '无'} | "
                    f"CPU备选: {plan.cpu_max_side}px"
                )
            else:
                self.precision_status_label.config(
                    text=f"{plan.gpu_name or 'GPU'} {plan.primary_max_side}px",
                    foreground="green"
                )
                detail = (
                    f"专用 {plan.dedicated_free_gib:.1f}G/{plan.dedicated_total_gib:.1f}G 空闲, "
                    f"共享 {plan.shared_available_gib:.1f}G 可用 | "
                    f"降级: {plan.gpu_fallback_sides or '无'} | "
                    f"CPU备选: {plan.cpu_max_side}px / ~{plan.cpu_est_seconds:.0f}s"
                )
        else:
            self.precision_status_label.config(
                text=f"CPU {plan.primary_max_side}px",
                foreground="darkorange"
            )
            detail = (
                f"GPU 显存不足 (专用 {plan.dedicated_free_gib:.1f}G/{plan.dedicated_total_gib:.1f}G, "
                f"共享 {plan.shared_available_gib:.1f}G 可用) | 预计 {plan.cpu_est_seconds:.0f}s"
            )
        self.precision_detail_label.config(text=detail)

    def _on_reprobe_precision(self):
        self.status_label.config(text="正在检测设备能力...", foreground="darkorange")
        self.root.after(100, self._do_reprobe)

    def _do_reprobe(self):
        self._probe_precision()
        if self._precision_plan is not None:
            self.status_label.config(text="设备检测完成", foreground="green")
            self.mark_inference_dirty()
        else:
            self.status_label.config(text="设备检测失败", foreground="red")

    def _on_precision_change(self):
        self.apply_deepsketch_precision()
        self.mark_inference_dirty()

    def _show_deepsketch_progress(self):
        self._deepsketch_progress = 0
        self.deepsketch_progress_var.set(0)
        self.deepsketch_progress_bar.pack(pady=(5, 0))
        self._poll_deepsketch_progress()

    def _hide_deepsketch_progress(self):
        if self._progress_poll_id is not None:
            self.root.after_cancel(self._progress_poll_id)
            self._progress_poll_id = None
        self.deepsketch_progress_var.set(100)
        self.root.after(800, lambda: self.deepsketch_progress_bar.pack_forget())

    def _poll_deepsketch_progress(self):
        val = getattr(self, "_deepsketch_progress", 0)
        self.deepsketch_progress_var.set(val)
        if val < 100:
            self._progress_poll_id = self.root.after(200, self._poll_deepsketch_progress)
        else:
            self._progress_poll_id = None

    def mark_inference_dirty(self, *args):
        if not self.image_path:
            return
        self._inference_dirty = True
        self.status_label.config(text='配置已变更，请点击"开始推理"', foreground="darkorange")

    def start_inference(self):
        if not self.image_path:
            self.status_label.config(text="请先选择图片", foreground="orange")
            return
        if not self._inference_dirty and self.contours:
            self.status_label.config(text="已有路径数据，可直接开始绘制", foreground="green")
            return
        self._inference_dirty = False
        self.status_label.config(text="正在推理，请等待...", foreground="darkorange")
        self.update_preview()
    ...
    def load_image(self):
        ...
        if file_path:
            ...
            self._inference_dirty = False
            onnx_enabled = self.color_to_sketch_var.get()
            mode = "quality" if (self.color_path_backend == "deepsketch_full" or self.deepsketch_enabled) else "fast"
            saved_config = self.load_inference_config_for_current_image(mode, onnx_enabled)
            if saved_config:
                self.apply_inference_config(saved_config)
                self.apply_deepsketch_precision()

            # Try to load paths from existing output
            loaded = self._try_load_contours_from_existing_output(onnx_enabled)
            if loaded:
                self.status_label.config(text='图片已载入，已恢复已有路径数据，可直接开始绘制', foreground="green")
            elif saved_config:
                self.status_label.config(text='图片已载入，已恢复上次推理配置，请确认后点击"开始推理"', foreground="green")
            else:
                self.status_label.config(text='图片已载入，请确认模式和精度后点击"开始推理"', foreground="green")

            self.show_selected_image_preview()

    def _try_load_contours_from_existing_output(self, onnx_enabled):
        import json as _json
        import os as _os
        suffix = "onnx_quality" if onnx_enabled else "quality"
        if getattr(sys, "frozen", False):
            base_dir = _os.path.dirname(sys.executable)
        else:
            base_dir = _os.path.dirname(_os.path.abspath(sys.argv[0]))
        image_name = _os.path.splitext(_os.path.basename(self.image_path))[0]
        safe_name = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in image_name).strip("_")
        if not safe_name:
            return False
        dir_name = f"{safe_name}_{suffix}"
        output_dir = _os.path.join(base_dir, "output", dir_name)
        high_quality_json = _os.path.join(output_dir, "ordered_paths_high_quality.json")
        if not _os.path.exists(high_quality_json):
            return False
        try:
        ...
        path_data = []
        path_meta_data = []
        for path, meta in zip(paths, path_metas):
            contour = np.array([[[p[0], p[1]]] for p in path], dtype=np.float64)
            path_data.append(contour)
            path_meta_data.append({
                "kind": meta.get("kind", "line"),
                "level": meta.get("level", "normal"),
                "length": self._path_length(path),
                "curvature": self._estimate_curvature(path),
                "source": meta.get("source", "skeleton"),
            })

        self.contours = path_data
        self.path_meta = path_meta_data
        self._inference_dirty = False

        quality_stats_path = _os.path.join(output_dir, "path_quality_stats.json")
        if _os.path.exists(quality_stats_path):
            try:
                with open(quality_stats_path, "r", encoding="utf-8") as f:
                    self.debug_deepsketch_stats = _json.load(f)
            except Exception:
                pass

        return True

    def show_selected_image_preview(self):
        if not self.image_path:
            return
        img = cv2.imdecode(np.fromfile(self.image_path, dtype=np.uint8), cv2.IMREAD_COLOR)
        if img is None:
            self.preview_canvas.delete("all")
            self.preview_label.config(text="Failed to load image preview")
            return

        self.image_size = (img.shape[1], img.shape[0])
        self._preview_image = None

        if self.contours and self.path_meta:
            # Show paths overlay when contours already loaded from existing output
            preview_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            h, w = img.shape[:2]
            overlay = np.zeros((h, w, 3), dtype=np.uint8)
            colors = {"outline": (0, 255, 0), "normal": (255, 255, 0), "detail": (0, 0, 255)}
            for contour, meta in zip(self.contours, self.path_meta):
                level = meta.get("level", "normal")
            ...
            blended = cv2.addWeighted(preview_rgb, 0.4, overlay, 0.6, 0)
            preview_img = Image.fromarray(blended)
        else:
            self.contours = []
            self.path_meta = []
            preview_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        if self.contours:
            self.preview_label.config(text=f"{os.path.basename(self.image_path)} (已有{len(self.contours)}条路径)")
        else:
            self.preview_label.config(text=os.path.basename(self.image_path))
    ...
    def update_preview(self, *args):
        ...
        use_quality = self.color_path_backend == "deepsketch_full" or self.deepsketch_enabled
        if use_quality:
            if self.color_to_sketch_var.get():
                self._set_status("正在生成 ONNX line_raw...")
                deepsketch_input = self.get_or_create_onnx_lineart(img, "quality")
                onnx_image = deepsketch_input
            else:
                self._set_status("正在使用原图运行 DeepSketch...")
                deepsketch_input = img
                onnx_image = None
            ordered_paths, ordered_metas = self.process_color_image_paths_deepsketch(
                img, deepsketch_input, onnx_image=onnx_image
            )
        elif self.color_to_sketch_var.get():
            ordered_paths, ordered_metas = self.process_color_image_paths(img)
        else:
            ordered_paths, ordered_metas = self._process_lineart_image_paths(img)

        if not ordered_paths:
            self.contours = []
            self.path_meta = []
            self.preview_canvas.delete("all")
            self.preview_label.config(text="未检测到有效线条，请调整阈值")
            return
        ...
        for path, meta in zip(ordered_paths, ordered_metas):
            ...
            path_meta_data.append({
                ...
                "length": self._path_length(path),
                "curvature": self._estimate_curvature(path),
                "source": meta.get("source", "skeleton") if isinstance(meta, dict) else "skeleton"
            })
        ...
        if self._preview_image is not None:
            preview_img = Image.fromarray(self._preview_image)
        else:
            preview_img = Image.fromarray(cv2.imdecode(np.fromfile(self.image_path, dtype=np.uint8), cv2.IMREAD_GRAYSCALE))
        preview_img.thumbnail((300, 200), Image.Resampling.LANCZOS)
        self.tk_img = ImageTk.PhotoImage(preview_img)
        self.preview_canvas.delete("all")
        self.preview_canvas.create_image(150, 100, anchor="center", image=self.tk_img)

        outline_count = sum(1 for m in ordered_metas
                           if (m.get("level") if isinstance(m, dict) else m) == "outline")
        normal_count = sum(1 for m in ordered_metas
                          if (m.get("level") if isinstance(m, dict) else m) == "normal")
        fill_count = sum(1 for m in ordered_metas
                        if (m.get("level") if isinstance(m, dict) else m) == "fill")
        detail_count = sum(1 for m in ordered_metas
                          if (m.get("level") if isinstance(m, dict) else m) == "detail")
        backend_label = ""
        if self.color_path_backend == "deepsketch_full" or self.deepsketch_enabled:
            stats = self.debug_deepsketch_stats
            if stats:
                backend_label = f" | DeepSketch {stats.get('model_size','full')} / {stats.get('optimized_points','?')}点"
            else:
                backend_label = " | DeepSketch full"
            q_step = getattr(self, "deepsketch_quality_draw_step", 1)
            backend_label += f" | 建议拖拽步长≤{q_step}"
        self.preview_label.config(
            text=f"解析完成：{len(ordered_paths)} 条 (轮廓{outline_count}/线{normal_count}/填充{fill_count}/细节{detail_count}){backend_label}"
        )
    ...
    def on_hotkey_start(self):
        if not self.is_running and self.contours:
            ...
            self.status_label.config(text="请先上传图片并完成解析，再按开始快捷键。", foreground="orange")

    def on_hotkey_pause(self):
        if self.is_running and not self.stop_requested:
            self.is_paused = not self.is_paused
        elif not self.is_running:
            self.status_label.config(text="当前未在绘制，无需暂停。", foreground="gray")
    ...
    def on_hotkey_stop(self):
        if self.is_running:
            ...
            self.status_label.config(text="当前未在绘制，无需停止。", foreground="gray")
    ...
    def draw_task(self):
        ...
        is_quality = (
            self.deepsketch_enabled
            or self.color_path_backend == "deepsketch_full"
            or any(m.get("source", "").startswith("deepsketch_svgpathtools")
                   for m in (self.path_meta or []))
        )
        if is_quality:
            base_step_px = quality_draw_step
        else:
            base_step_px = max(1, self.drag_step_var.get())
        base_delay = max(0.015, self.delay_var.get())
        mouse_btn = self.btn_var.get()
        ...
        try:
            ...
            for idx, contour in enumerate(self.contours):
                ...
                meta = self.path_meta[idx] if idx < len(self.path_meta) else None
                level = meta.get("level") if meta else "normal"
                if level == "detail":
                    step_px = 1
                    delay = max(base_delay, 0.025)
                elif level == "fill":
                    step_px = max(1, min(base_step_px, 3))
                    delay = max(base_delay, 0.020)
                elif level == "outline":
                    step_px = max(1, min(base_step_px, 2))
                    delay = base_delay
                else:
                    step_px = max(1, min(base_step_px, 2))
                    delay = base_delay
...
def run_deepsketch_worker():
    ...
    repo = bundled_path(os.path.join("models", "deepsketch"))
    script = os.path.join(repo, "predict_s1.py")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--deepsketch-worker":
        run_deepsketch_worker()
        sys.exit(0)
