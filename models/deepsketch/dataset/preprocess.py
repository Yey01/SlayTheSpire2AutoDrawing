...
def rasterize(svg, png, dpi = 400):
    ...
    if which("inkscape") is not None:
        ...
        try:
            import cairosvg
            cairosvg.svg2png(url=svg, write_to=png, dpi=dpi, background_color="white")
            return
        except Exception:
            pass
        ...
        def _svg_size(value, default):
            if value is None:
                return default
            match = re.search(r"[-+]?\d*\.?\d+", str(value))
            return int(round(float(match.group(0)))) if match else default

        paths, attrs, svg_attrs = svg2paths2(svg)
        width = _svg_size(svg_attrs.get("width"), None)
        height = _svg_size(svg_attrs.get("height"), None)
        if width is None or height is None:
            view_box = svg_attrs.get("viewBox") or svg_attrs.get("viewbox")
            if view_box:
                nums = [float(x) for x in re.findall(r"[-+]?\d*\.?\d+", view_box)]
                if len(nums) >= 4:
                    width = int(round(nums[2]))
                    height = int(round(nums[3]))
        width = max(width or 512, 1)
        height = max(height or 512, 1)

        canvas = np.full((height, width), 255, dtype=np.uint8)
        for path_obj, attr in zip(paths, attrs):
            stroke_width = max(_svg_size(attr.get("stroke-width"), 1), 1)
            length = max(float(path_obj.length(error=1e-2)), 1.0)
            steps = max(int(length / 1.0), 2)
            prev = path_obj.point(0)
            for i in range(1, steps + 1):
                cur = path_obj.point(i / steps)
                p0 = (int(round(prev.real)), int(round(prev.imag)))
                p1 = (int(round(cur.real)), int(round(cur.imag)))
                cv2.line(canvas, p0, p1, 0, stroke_width, lineType=cv2.LINE_AA)
                prev = cur
